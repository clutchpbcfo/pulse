...
const CFG = { basePoints:10, ringTargetBase:8, baseSpeed:1.75, speedPerLock:0.05, speedPerTier:0.55, speedCap:5.0, zoneBase:0.92, zoneMin:0.34, zoneShrinkPerTier:0.075, perfectFrac:0.34, minTravel:1.9, epochUTC: Date.UTC(2026,5,2) };
const TAU = Math.PI * 2;
...
function simulate(seed, taps){
  ...
  const place = (angle,dir)=> wrap(angle + dir*(CFG.minTravel + rng()*(TAU - CFG.minTravel*2)));
  ...
  for(let i=0;i<taps.length;i++){
    ...
    if(d <= zone/2){
      ...
      if(locks >= ringTarget){
        ...
        target = place(angle,dir);
      } else { target = place(angle,dir); }
    } else {
      if(shields>0){ shields--; combo=0; target = place(angle,dir); }
